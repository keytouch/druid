...
public class GroupByStrategyV2 implements GroupByStrategy
{
  ...
  )
  {
    ...
    if (hasTimestampResultField) {
      ...
      final Object timestampResultFieldGranularityValue = query
          .getContextValue(GroupByQuery.CTX_TIMESTAMP_RESULT_FIELD_GRANULARITY);
      if (timestampResultFieldGranularityValue == null) {
        timestampResultFieldGranularity = null;
      } else if (timestampResultFieldGranularityValue instanceof String) {
        final String granularityString = (String) timestampResultFieldGranularityValue;
        try {
          timestampResultFieldGranularity = jsonMapper.readValue(granularityString, Granularity.class);
        } catch (IOException e) {
          throw new IAE(StringUtils.format(
              "Expected context key [%s] to be %s, but got [%s]",
        ...
        throw new IAE(StringUtils.format(
            ...
            GroupByQuery.CTX_TIMESTAMP_RESULT_FIELD_GRANULARITY,
            "a String",
            timestampResultFieldGranularityValue.getClass().getName()));
      }
      dimensionSpecs =
          query.getDimensions()
               .stream()
